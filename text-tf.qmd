...
This is similar but not exactly the same as [Range Scaling](@sec-numeric-range), as we are making it so the values across the row sum to 1, not making it so each column has values between 0 and 1.
Since we are working with counts, there are also times when log normalization is done, which means that we take the columns produced here, and take the [logarithm](@sec-numeric-logarithms) of them, Typically with an offset because this method tends to produce a lot of zeroes.
...
Any such variable should properly be removed anyway as we further talk about in the [Zero Variance](@sec-too-many-zv) chapter.
