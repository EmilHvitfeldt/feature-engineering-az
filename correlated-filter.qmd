...
#| tbl-cap: Some predictors appears multiple times in this table.
ames |>
  select(where(is.numeric)) |>
  correlate(method = "pearson", quiet = TRUE) |>
  shave() |>
...
One way to do filtering is to pink a threshold and repeatably remove one of the variables of the most correlated pair, until there are no pairs left with an correlation over the threshold. This method has a minimal computational footprint as it just needs to calculate the correlations once at the beginning. The threshold is likely to be needed to be tuned as we can't say for sure what a good threshold is. With removal of variables there is always a chance that we are removing signal rather than noise, this is increasingly true as we remove more and more predictors.

TODO: rewrite this as an algorithm
...
TODO: find different example so `Sale_Price` isn't part of this as it is usually the outcome.

TODO: Add good graph showing this effect.

What we could do, is take the correlation matrix and apply a clustering model on it. Them we use the clustering model to lump together the groups of highly correlated predictors. Then within each cluster one predictor is chosen to be kept. The clusters should ideally be chosen such that uncorrelated predictors are alone in their cluster. This method is able to work better with the global structure of the data, but requires fitting and tuning another model.
...
- Will lead to faster and simpler model
...
We can see that when we lower this threshold to the extreme, that more predictors are removed.
