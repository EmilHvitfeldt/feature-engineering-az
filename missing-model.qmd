...
TODO: figure out if MICE is applicable here
...
Below we are showing how we can impute using a K-nearest neighbor model using `step_impute_knn()`. We are specifying the variable to impute on first, then with `impute_with` we specify which variables are used as predictors in the model.
