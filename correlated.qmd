...
#|   predictor_2. The first chart shows points uniformally spread. With each of
#|   the following charts having stronger and stronger trend between the two
#|   predictors.
library(ggplot2)
library(dplyr)
library(patchwork)
...
We will see how we can example the correlation structure to figure out which variables we can eliminate, this is covered in @sec-correlated-filter. This is a more specialized version of the methods we cover in @sec-too-many-filter as we are looking at correlation to determine which variablees to remove rather then their relationship to the outcome.

Another method that works well in anything PCA related, these are covered in @sec-too-many-pca and @sec-too-many-pca-variants. The resulting data coming out of PCA will be uncorrelated.
