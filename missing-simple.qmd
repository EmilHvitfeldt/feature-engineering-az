...
image: social-cards/missing-simple.png
---
...
#| fig-alt: |
#|   Tile heatmap. Total rooms (x-axis) vs bedrooms (y-axis) from Ames data.
#|   Color intensity shows count (log scale). A diagonal line marks where
#|   bedrooms equal total rooms. All observations fall below this line (fewer
#|   bedrooms than total rooms). Most houses cluster around 6 total rooms
#|   and 3 bedrooms.
library(tidymodels)
ames |>
...
#| fig-alt: |
#|   Faceted histogram with 4 panels showing mean imputation effects. 0%:
#|   original bimodal distribution (peaks at 100 and 150). 5%, 10%, 15%:
#|   same distribution but with progressively taller spike at the mean (~117).
set.seed(1234)
