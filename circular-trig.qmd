...
One of the most common ways of dealing with circular data is by applying the trigonometric functions `sin()` and `cos()` to the circular variable. 
...
using appropriate cycle size and frequency we can overlay them on the previous chart
...
The starting point is also important.
...
recipe(target ~ predictor, toy_data_two) |>
    ...
    prep() |>
    bake(new_data = NULL) |>
    rename(`sin(365)` = predictor_sin_one, `cos(200)` = predictor_sin_two) |>
    pivot_longer(-c(predictor, target)) |>
    ggplot(aes(predictor, value, color = name)) +
    geom_point(aes(predictor, target), color = "black") +
    geom_line() +
