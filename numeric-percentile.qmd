# Percentile {#sec-numeric-percentile}

**Percentile scaling** (also sometimes called **Rank Scaling**) is a method where we apply a non-linear transformation to our data where each value is the percentile of the training data.
...
Notice how there are only `r nrow(tidy(percentile500_rec, 1))` values in this output. This is happening because some percentile has been collapsed to save space since if the value for the 10.4 and 10.6 percentile is the same, we just store the 10.6 value.
