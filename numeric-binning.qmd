...
image: social-cards/numeric-binning.png
---
...
#|   follows a sinusoidal curve: low at extremes, peaking around predictor
#|   values 25-50.
library(ggplot2)
...
#| fig-alt: |
#|   Faceted line chart with 4 panels, one per bin. Predictor on x-axis,
#|   binary indicator (0 or 1) on y-axis. Each bin activates (value=1) for
#|   approximately one quarter of the predictor range and is zero elsewhere.
#|   Bins progress left to right: Bin 1 covers lowest values, Bin 4 highest.
data_bins |>
  select(-outcome) |>
...
#| fig-alt: |
#|   Faceted scatter chart with 4 panels, one per bin. Each shows the same
#|   sinusoidal predictor-outcome relationship. Color intensity (white to
#|   dark blue) shows bin activation. The peak outcome region (predictor
#|   25-50) is split between Bin 2 and Bin 3.
data_bins |>
...
#| fig-alt: |
#|   Scatter chart with step function overlays. Points show sinusoidal
#|   predictor-outcome relationship. Four step functions (3, 5, 7, 9 bins)
#|   attempt to fit the data. More bins produce finer steps that better
#|   approximate the smooth curve, but all show characteristic staircase
#|   pattern that cannot perfectly match the continuous relationship.
library(tidymodels)
...
#| fig-alt: |
#|   Scatter chart showing data with two clear discontinuities around
#|   predictor values 40 and 80. Step function fits with 3 and 4 evenly-spaced
#|   bins are overlaid but perform poorly because the bin boundaries don't
#|   align with the actual breaks in the data.
library(tidymodels)
...
#|   right places.
#| fig-alt: |
#|   Scatter chart with same discontinuous data. CART-based discretization
#|   finds optimal cut points that align with the natural breaks in the data.
#|   The step functions now correctly capture the three distinct levels,
#|   demonstrating the advantage of supervised binning over equal-width bins.
library(tidymodels)
library(embed)
