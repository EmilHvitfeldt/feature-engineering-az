...
**Binning**, is one way to represent a curve that makes it useful in a modeling content, since it allows us to model non-linear relationships between predictors and outcomes. This is a trained method.
...
Below we see the way each of the new variables activete different areas of the domain. 
...
What we see in this example that the regions appears to be evenly spaces over the observed values of the predictor. This is neither and good or bad thing, but something we need to think about. Ideally we want the different regions to represent something shared within them. If the cuts are happening at non-ideal locations we don't get the full benefit. See below for such an example 
...
Below is a chart of how well using evenly split binning works when using it on our toy example. We notice that using binning doesn't really work that well for this type of data. And it makes sense, binning will naturally create a step function like we see below, and if your predictor doesn't have that shape, you are unlikely to see as good performance as with splines @sec-splines or polynomial expansion @sec-polynomial.
...
#| message: false
#| fig-cap: |
#|   The evenly spaced cuts does not perform well when the breaks in the data
#|   set isn't evenly spaced.
#| fig-alt: |
#|   Scatter chart. Predictor along the x-axis and outcome along the y-axis.
#|   The data has some wiggliness to it with two big breaks in the curve. Both 
#|   of the fitted lines doesn't match the data, as the breaks doens't align
#|   with the data set.
library(tidymodels)
...
While these results looks promising, there are a couple of things that are worth remembering. Firstly, this is not a silver bullet, as we still need to tune the method, to find the optimal value of for the model we are fitting. Secondly, this type of data is not seen that much in the wild, and if you are seeing them in your application, there is a good change that you can manually encode this information without having to resort to these methods. 
