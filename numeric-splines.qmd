...
As we saw in the [Binning](@sec-numeric-binning) chapter, one way to deal with this non-linearity is to chop up the predictor and emit indicators for which region the values take.
...
If you are using a model type that doesn't handle correlated features well, then you should take a look at the methods outlined in the [Correlated section](@sec-correlated) for ways to deal with correlated features.
