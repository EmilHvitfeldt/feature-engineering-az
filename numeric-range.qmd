...
image: social-cards/numeric-range.png
---
...
#| fig-alt: |
#|   Scatter chart showing range scaling with clipping. Original values on
#|   x-axis (0-50), transformed values on y-axis (0-1). Three distinct regions:
#|   values below 10 map to 0 (left plateau), values 10-32 follow a linear
#|   diagonal, values above 32 map to 1 (right plateau).
library(recipes)
library(ggplot2)
...
#| fig-alt: |
#|   Four histograms in a 2x2 grid. Rows: Original and Transformed. Columns:
#|   No outlier and One outlier. Left column shows normal range scaling of
#|   bimodal data to [0,1]. Right column shows same data with one extreme
#|   outlier, which compresses the transformed distribution to near-zero
#|   width since the outlier dominates the range calculation.
library(ggplot2)
library(dplyr)
library(tidyr)
set.seed(1234)
