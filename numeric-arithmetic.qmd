...
image: social-cards/numeric-arithmetic.png
---
...
#| fig-alt: |
#|   Two scatter charts side by side. Left: deposits vs withdrawals with color
#|   encoding outcome - high outcome (yellow) appears when deposits are high
#|   and withdrawals are low. Right: ratio (deposits/withdrawals) vs outcome
...
#| fig-alt: |
#|   Two scatter charts side by side. Left: deposits vs withdrawals with color
#|   encoding outcome - high outcome (yellow) appears when both deposits AND
#|   withdrawals are high. Right: product (deposits*withdrawals) vs outcome
#|   shows a clear linear relationship.
set.seed(12345)
library(tidyverse)
library(patchwork)
