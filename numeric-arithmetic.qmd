...
maybe show how log and logit can influence the scales and how they interact with ratios
:::
...
TODO fix below to actually show something
...
ratio_data <- tibble(
  deposits = sample(seq(100, 500), 1000, TRUE),
  withdrawals = sample(seq(100, 500), 1000, TRUE)
) |>
  mutate(
    outcome = case_when(
      deposits > 300 & withdrawals > 300 ~ 1,
      deposits < 300 & withdrawals < 300 ~ 1,
      .default = 0
    ),
    outcome = outcome + rnorm(1000, sd = 0.2)
  ) |>
  filter(
    abs(withdrawals - 300) > 25,
    abs(deposits - 300) > 25
  ) |>
  mutate(
    deposits = deposits + rpois(n(), 45),
    withdrawals = withdrawals + rpois(n(), 45)
  )
...
we can thing of ratios and crosses as identical, once we ally inversion of variables
