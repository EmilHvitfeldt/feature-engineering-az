...
image: social-cards/outliers.png
---
...
#| fig-alt: |
#|   Scatter chart of Ames housing data. Living area (sq ft) on x-axis, sale
#|   price (USD) on y-axis. Main cluster shows positive correlation along a
#|   diagonal. Two outlier groups are highlighted with rectangles: three points
#|   at far right with very large living area but unusually low prices (partial
#|   sales), and two points with both high living area and high prices (large
#|   but appropriately priced homes).
library(modeldata)
library(dplyr)
library(ggplot2)
library(ggforce)
...
This will very briefly be covered in the [outliers identify](outliers-identify.qmd) chapter.
The remaining chapters will cover how we can deal with the outliers once we find them.
In the chapter [Outlier removal](outliers-remove) we look at how we can remove outliers.
