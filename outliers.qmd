...
Some other preprocessing method such as [Robust Scaling](@sec-numeric-robust) also isn't affected by outliers.

If you are planning on handling outliers you want to start by identifying them.
In the [Outlier removal](@sec-outliers-remove) chapter we look at how we can identify and remove outliers.
In the [outliers issues](@sec-numeric-outliers-issues) section we cover numerical transformations that lessen the effect that outliers have on the distribution.
...
We look at methods on how to do that in the [Outliers Imputation](@sec-outliers-imputation) chapter.
In addition to all of these methods, we can also add additional indicator variables to denote whether an observation is an outlier or not, this is covered in the [Outliers Indicate](@sec-outliers-indicate) chapter.
