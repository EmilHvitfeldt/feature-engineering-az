...
Methods that take many variables and return fewer variables such as dimensionality reduction methods will be covered in the [Too Many variables section](@sec-too-many).
...
The power transformations we saw in the [Box-Cox](@sec-numeric-boxcox) and [Yeo-Johnson](@sec-numeric-yeojohnson) chapters, distort the transformations, where these feature scalings essentially perform a "zooming" effect.
...
Centering, scaling, and normalization will all be discussed in the [Normalization](@sec-numeric-normalization) chapter.
