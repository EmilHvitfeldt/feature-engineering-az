...
The main difference between the methods is how $a$ and $b$ are calculated. These methods are learned transformation. So we use the training data to derive the right values of $a$ and $b$, and then these values are used to perform the transformations when applied to new data. The different methods might differ on what property is desired for the transformed variables, same range or same spread, but they never change the distribution itself. The power transformations we saw in @sec-numeric-boxcox and @sec-numeric-yeojohnson, distort the transformations, where these feature scalings essentially perform a "zooming" effect.
...
We see here that all the methods in @tbl-feature-scaling follows the equation @eq-feature-scaling. Sometimes $a$ and $b$ takes a value of 0, which is perfectly fine. Centering and scaling when used together is equal to normalization. They are kept separate in the table since they are sometimes used independently. Centering, scaling, and normalization will all be discussed in @sec-numeric-normalization.
...
Show different distributions, and how well the different methods does at dealing with them
:::
