...
So for example the [Numeric section](@sec-numeric) contains methods that deal with numeric variables such as [Logarithms](@sec-numeric-logarithms) and and [Max Abs Scaling](@sec-numeric-maxabs), and the [Categorical section](@sec-categorical) contains methods that deal with categorical variables such as [Dummy Encoding](@sec-categorical-dummy) and [Hashing Encoding](@sec-categorical-hashing).
...
We see this in the [Normalization](@sec-numeric-normalization) chapter, where we explore centering and scaling.
...
Examples of untrained methods are [logarithmic transformation](@sec-numeric-logarithms) and [datetime value extraction](@sec-datetime-extraction).
...
Some untrained methods have a high computational cost, such as [BERT](@sec-text-bert).
...
When selecting these features to keep in a feature selection routine as seen in the [Too Many variables section](@sec-too-many) is a good example.
...
In the case of centering of numeric variables described in the [normalization](@sec-numeric-normalization) chapter, you need to know the mean value of the training data set to apply this transformation.
...
In contrast, taking the square root of a variable as described in the [Square root](@sec-numeric-sqrt) chapter isn't a learned method as there isn't any sufficient information needed.
