...
As we saw in @sec-logarithms about logarithms, we sometimes have to deal with highly skewed data. Square roots are another way to deal with this issue, with some different pros and cons that make it better to use us in some situations. We will spend our time in this section to talk about what those are.
...
This variable contains some negative values with the smallest being `r min(hotel_bookings$adr)`. We wouldn't want to throw out the negative values. And we could think of many situations where both negative and positive values are part of a skewed distribution, especially financial. Bank account balances, delivery times, etc etc.
