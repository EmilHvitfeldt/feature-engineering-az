...
The feature engineering methods we have seen so far, have been counting based or variations thereof as seen in @sec-text-tf, @sec-text-tfidf, and @sec-text-hashing. Latent Dirichlet Allocation (LDA) is different because it is done by first fitting a model on the data, and then using this fitted model to define features for us.
...
Documents, which are the established terminology for the observations, are seen as a bag-of-words representation of tokens. Meaning that much like @sec-text-tfidf, the order doesn't matter. We are simply looking at token counts.
