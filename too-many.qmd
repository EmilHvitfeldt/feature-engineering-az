...
We look at ways to deal with [correlated features](@sec-correlated), but there will also be methods here to accomplish similar goals.
...
We look at these methods in the [Non-zero Variance filtering](@sec-too-many-zv) chapter.
...
[Filter-based approaches](@sec-too-many-filter), these methods give each feature a score or rank, and then you use this information to select variables.
There are many different ways to get these rankings and many will be covered in the chapter.
[Wrapper-based approaches](@sec-too-many-wrapper).
...
Lastly, we have [embedded methods](@sec-too-many-embedded).
