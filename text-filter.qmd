...
This chapter is going to be quite similar to the [stop word removal](@sec-text-stopwords) chapter.
...
For counting methods such as [TF](@sec-text-tf) and [TF-IDF](@sec-text-tfidf), each unique token leads to 1 column in the output.
...
This is very similar to what some people do as a proxy for [stop word removal](@sec-text-stopwords).
...
We will talk about [TF-IDF](@sec-text-tfidf), but that can also be helpful in finding noninformative words.
