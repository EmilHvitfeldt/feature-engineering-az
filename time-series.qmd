...
Some of these transformations can be taken from @sec-numeric.
However, some metrics are specific to time series data.
Likewise, some metrics are similar to what we do in @sec-missing and @sec-outliers, but special care needs to be taken with time series data.
...
Particularly holiday extraction as seen in @sec-datetime-extraction.
