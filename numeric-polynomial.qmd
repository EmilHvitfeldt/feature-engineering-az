...
As we saw in the [Binning](@sec-numeric-binning), one way to deal with this non-linearity is to chop up the predictor and emit indicators for which region the values take.
...
The interpretation of these polynomial features is not as easy as with [Ninning](@sec-numeric-binning) or [Splines](@sec-numeric-splines), but the calculations are quite fast and versatile.
