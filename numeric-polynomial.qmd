# Polynomial {#sec-numeric-polynomial}
...
As we saw in @sec-numeric-binning, one way to deal with this non-linearity is to chop up the predictor and emit indicators for which region the values take. While this works, we are losing quite a lot by detail by the rounding that occurs.
...
The interpretation of these polynomial features are not as easy as with binning in @sec-numeric-binning or splines @sec-numeric-splines, but the calculations are quite fast and versatile.
