...
In @sec-text-tf, @sec-text-tfidf, and @sec-text-hashing we took a *bag of words* approach to encoding the tokens into numeric columns.
...
That doesn't mean that filtering tokens shouldn't be done, just that it isn't a requirement as it often is for methods such as @sec-text-tf and @sec-text-tfidf.
