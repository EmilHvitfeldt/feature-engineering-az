...
One of the nice things about this method is that you aren't restricted to a certain number of unique tokens as they just influence the number of unique values the columns can take, not the number of columns. That doesn't mean that filtering tokens shouldn't be done, just that it isn't a requirement as it often is for methods such as @sec-text-tf and @sec-text-tfidf.
