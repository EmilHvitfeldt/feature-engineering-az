...
Secondly, some classes have different numbers of observations associated with them.
...
The way we handle unseen levels gives us a clue as to how we can deal with low-frequency counts.
...
This smoothing isn't required but it is often beneficial to include if you are dealing with low count levels.
It is also recommended because if omitted, it would lead to overfitting since we are essentially using the outcome to guide the predictors.
...
The embed package comes with a couple of functions to do target encoding.
