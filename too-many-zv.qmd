...
And are sometimes created in the course of the feature engineering process, such as when we do [Dummy Encoding](@sec-categorical-dummy) on categorical predictors with known possible levels.
...
Take the normalization methods in the [Normalization](@sec-numeric-normalization) chapter, some of these require division with the standard deviation, which is zero thus resulting in division by 0.
Other methods like [PCA](@sec-too-many-pca) can get in trouble as zero variance predictors can yield non-invertible matrices that they can't normally handle.
