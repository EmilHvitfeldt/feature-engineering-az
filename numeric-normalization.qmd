...
image: social-cards/numeric-normalization.png
---
...
#|   original bimodal distribution from a beta distribution. Bottom panel shows
#|   the same data after normalization (mean=0, sd=1), retaining the
#|   bimodal shape. A green normal curve overlay on the bottom panel clearly
#|   does not match.
library(ggplot2)
library(dplyr)
library(tidyr)
...
#|   No outlier and One outlier. Left column shows normal normalization of
#|   bimodal data. Right column shows the same data with one extreme outlier
#|   added, which compresses the transformed distribution to about one-fifth
#|   the width.
library(ggplot2)
library(dplyr)
library(tidyr)
